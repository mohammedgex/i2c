...
class LanguageListController extends GetxController {
  ...
  var languageResponse = Rxn<LanguageResponseModel>();  // Change to typed response model
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      dio.Response? response = await _apiService.getData(
        url: ApiEndpoint.languageListUrl, // Replace with your API endpoint
        requiresAuth: false, // Set this to false if authentication is not needed
      );

      if (response != null && response.statusCode == 200) {
        // Map the response to LanguageResponseModel
        languageResponse.value = LanguageResponseModel.fromJson(response.data);
